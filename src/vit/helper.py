'''
We define a custom trainer that uses streaming datasets to load the imagenet dataset.
This is because the imagenet dataset is too large to fit in memory.
'''
...
class CustomTrainer(Trainer):
    ...
    def get_train_dataloader(self):
        if self.dataloader_calls == 0:
            return super().get_train_dataloader()
        else:
            # Reinitialize the streaming dataset
            self.train_dataset = prepare_imagenet_dataset(self.tokenizer, split='train').shuffle(seed=self.dataloader_calls)
            return super().get_train_dataloader()
