...
class CKA():
    ...
    def cka_alternative(self, gram_x, gram_y, debiased=False):
        """Compute CKA.
        ...
        eigval_x, eigvec_x = np.linalg.eigh(gram_x)
        eigval_y, eigvec_y = np.linalg.eigh(gram_y)

        # Sort eigvals in descending order.
        sorted_indices_x = torch.argsort(eigval_x, descending=True)
        sorted_eigvals_x = eigval_x[sorted_indices_x]
        sorted_eigvecs_x = eigvec_x[:, sorted_indices_x]

        sorted_indices_y = torch.argsort(eigval_y, descending=True)
        sorted_eigvals_y = eigval_y[sorted_indices_y]
        sorted_eigvecs_y = eigvec_y[:, sorted_indices_y]

        denom = torch.sqrt(torch.sum(sorted_eigvals_x) * torch.sum(sorted_eigvals_y))
        eigval_prod = sorted_eigvals_x @ sorted_eigvals_y.T
        dot_prods = torch.matmul(sorted_eigvecs_x.t(), sorted_eigvecs_y)
        scaled = torch.sum(dot_prods * eigval_prod) / denom
        return scaled
    ...
    def linear_cka(self, features_x, features_y,debiased=False, alternative=False):
